...
def get_prm_files(ifg_dir):
    ...
    print(prm_files)
    print(os.path.splitext(os.path.basename(prm_files[0]))[0].isdigit())
    prm_files = [i for i in prm_files] 
    if len(prm_files) != 2:
...
def prepare_geometry(geom_files, meta, update_mode=True):
    ...
    for geom_file in geom_files:
        ...
        print(geom_file)
        geom_meta.update(readfile.read_gdal_vrt(geom_file[0]))

        # write .rsc file
        rsc_file = geom_file[0]+'.rsc'
...
def main(iargs=None):
    ...
    print(template['mintpy.load.demFile'])
    inps.dem_file = glob.glob(template['mintpy.load.demFile'])

    # extract common metadata
    rsc_file = os.path.join(inps.mintpy_dir, 'inputs/data.rsc')
    print(inps.unw_files)
